...
| SI-010 | Enable social cards (Material `social` plugin) | Medium | S | Low | [x] | 2026-07-14 | Unblocked via MSYS2 (not GTK3 runtime as D-028 suggested): installed MSYS2, ran `pacman -S mingw-w64-ucrt-x86_64-cairo` in the UCRT64 shell, added `C:\msys64\ucrt64\bin` to PATH; `python -c "import cairosvg"` then succeeded. `social` plugin re-added to `mkdocs.yml`, `use_material_social_cards: true` restored in `rss` config; `mkdocs build --strict` clean, 125 card PNGs generated, spot-checked home page and a long-title manual chapter — brand color, title wrapping, and truncated description all render correctly; from execution plan Item 4 |
...
| SI-012 | Add Google Search Console verification file | Low | S | Low | [x] | 2026-07-14 | `docs/google7edd60ce4d78d84d.html` (site owner started GSC property setup, dropped the verification file in `temp/`, copied to `docs/` to publish at site root); site owner verified in GSC and submitted `sitemap.xml` (127 URLs). Bing Webmaster Tools verification/submission tracked separately as SI-013; from execution plan Item 8 |
| SI-013 | Add Bing Webmaster Tools verification file | Low | S | Low | [x] | 2026-07-14 | `docs/BingSiteAuth.xml` (site owner used the XML-file verification method, dropped the file in `temp/`, copied to `docs/` to publish byte-identical at site root, confirmed via diff); site owner still needs to click Verify in Bing Webmaster Tools and submit the sitemap; from execution plan Item 8 |
...
| ID | Item | Priority | Effort | Risk | Status | Completed | Notes |
|---|---|---|---|---|---|---|---|
| SI-003 | Content templates — iterative | Medium | S | Low | [x] | 2026-07-14 | Project template updated 2026-04-20 with 4 project types, At a Glance box, Scope & Limitations, image conventions, pub checklist, hub guidance. 2026-07-14: added a "Before publishing" checklist block to the article template (previously had none) and added two items to both templates' checklists — meta description quality, checked for cross-links to related pages. Later same day: brought article template to parity with project template — added an optional At a Glance box (setup guides/cheatsheets), a screenshot/image-conventions note, an optional Scope & Limitations section (deep-dive articles), and a closing-paragraph + "← Back to [cluster]" nav-link convention (new for articles — no existing published article uses it yet, applies going forward); checklist extended to match. Then reordered the project template's own checklist to match the article template's item order and added the missing "placeholders filled in" check (project template didn't have it, article's did) — both checklists now share identical structure and ordering, only the wording differs (project/cluster nouns vs. article/cluster nouns). Closes the specific gap noted earlier — future article-specific needs can reopen as a new SI item. |
